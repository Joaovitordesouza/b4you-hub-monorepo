...
export interface EvolutionChat {
  ...
  lastMessageTimestamp?: number; // Backend Spec v2
  lastMessageTimestampMillis?: number; // Legacy/Fallback
  unreadCount: number;
  type: 'private' | 'group';
  leadId?: string; 
  leadName?: string;
...
export interface Lead {
  ...
  onboardingStatus?: OnboardingStage; // Atualizado para usar OnboardingStage
...
export interface ProducerStats {
  ...
  vendas_count: number;
  health_score: number;
  status_health: HealthStatus; // Atualizado
  ultima_venda: string;
...
export interface Producer {
  ...
  tracking_status?: TrackingStatus | null; // Nullable: se null, não está no tracking
  tracking_metadata?: TrackingMetadata; // Novo campo para controle fino
  ...
  // Controle de SLA e Responsáveis
  lastContactAt?: string;
  nextCheckinAt?: string;
  responsibleInternalId?: string; // ID do usuário interno responsável pelo setup
  responsibleClientId?: string;   // Nome/Contato do responsável no cliente
...
export interface Launch {
  ...
  integrations: string[]; // ['Shopify', 'ERP', ...]
  ...
  testDeadline: string; // Auto-calculated
